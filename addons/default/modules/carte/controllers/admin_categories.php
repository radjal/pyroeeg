...
class Admin_Categories extends Admin_Controller
{
	 ...
	 * - load the carte_categories model.
	 * - load the categories and carte language files.
	...
	public function index()
	{
		...
		$total_rows = $this->carte_categories_m->count_all();
		$pagination = create_pagination('admin/carte/categories/index', $total_rows, Settings::get('records_per_page'), 5);

		// Using this data, get the relevant results
		$categories = $this->carte_categories_m
								->order_by('title')
	...
	public function create()
	{
		...
		{
			if ($id = $this->carte_categories_m->insert($this->input->post()))
			{
				// Fire an event. A new carte category has been created.
				Events::trigger('carte_category_created', $id);
			...
			redirect('admin/carte/categories');
		}
	...
	{
		// Get the category
		$category = $this->carte_categories_m->get($id);

		// ID specified?
		$category or redirect('admin/carte/categories/index');
		...
		if ($this->form_validation->run())
		{
			$this->carte_categories_m->update($id, $this->input->post())
			...
			// Fire an event. A carte category is being updated.
			Events::trigger('carte_category_updated', $id);

			redirect('admin/carte/categories/index');
		}
	...
	public function delete($id = 0)
	{
		...
		if (!empty($id_array))
		{
			...
			{
				if ($this->carte_categories_m->delete($id))
				{
					$deleted++;
					$deleted_ids[] = $id;
				}
			...
			Events::trigger('carte_category_deleted', $deleted_ids);
		}
		else
		{
	...
	{
		if ($this->carte_categories_m->check_title($title, $this->input->post('id')))
		{
	...
	{
		if ($this->carte_categories_m->check_slug($slug, $this->input->post('id')))
		{
	...
	public function create_ajax()
	{
		...
		if ($this->form_validation->run())
		{
			$id = $this->carte_categories_m->insert_ajax($this->input->post());
