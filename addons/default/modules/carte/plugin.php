 ...
 * 
 * @author   Radja Lomas
 * @package  PyroCMS\Core\Modules\Carte\Plugins
 */
class Plugin_Carte extends Plugin
{
	...
	public $description = array(
		'en' => 'A plugin to display information such as carte categories and posts.',
        'fr' => 'Un plugin permettant d\'afficher des informations comme les catégories et articles du carte.'
	);
		...
		$info = array(
			'posts' => array(
				'description' => array(// a single sentence to explain the purpose of this method
					'en' => 'Display carte posts optionally filtering them by category.',
                    'fr' => 'Permet d\'afficher des articles de carte en les filtrants par catégorie.'
				),
			...
			'categories' => array(
				'description' => array(
					'en' => 'List carte categories.',
                    'fr' => 'Lister les catégories du carte'
				),
				'single' => false,
				'double' => true,
			...
			'count_posts' => array(
				'description' => array(
					'en' => 'Count carte posts that meet the conditions specified.',
                    'fr' => 'Permet de compter les articles de carte qui remplissent certaines conditions spécifiées.'
				),
				'single' => true,
				'double' => false,
				'variables' => '',
			...
			'tags' => array(
				'description' => array(
					'en' => 'Retrieve all tags that have been applied to carte posts.',
	 ...
	 * Carte List
	 *
	 * Creates a list of carte posts. Takes all of the parameters
	...
	public function posts()
	{
		...
		if ($category_string = $this->attribute('category'))
		{
			...
			{
				$cate_filter_by[] = '`'.$this->db->dbprefix('carte_categories').'`.`'.(is_numeric($category) ? 'id' : 'slug').'` = \''.$category."'";
			}
		...
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('carte_categories.title', true)." as 'category_title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('carte_categories.slug', true)." as 'category_slug'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('carte_categories.title', true)." as 'category||title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('carte_categories.slug', true)." as 'category||slug'";
		$this->row_m->sql['join'][] = 'LEFT JOIN '.$this->db->protect_identifiers('carte_categories', true).' ON '.$this->db->protect_identifiers('carte_categories.id', true).' = '.$this->db->protect_identifiers('carte.category_id', true);
		...
		// Return our content.	
		return $this->streams->parse->parse_tag_content($this->content(), $posts, 'carte', 'cartes', $loop);
	}

	/**
	 * Categories
	 *
	 * Creates a list of carte categories
	 *
	 * Usage:
	 * {{ carte:categories order-by="title" limit="5" }}
	...
	public function categories()
	{
		...
		$categories = $this->db
			...
			->limit($limit)
			->get('carte_categories')
			->result();

		foreach ($categories as &$category)
		{
			$category->url = site_url('carte/category/'.$category->slug);
		}
	...
	public function count_posts()
	{
		...
		return $this->db->count_all_results('carte');
	}
	
	/**
	 * Tag/Keyword List
	 *
	 * Create a list of carte keywords/tags
	 *
	 * Usage:
	 * {{ carte:tags limit="10" }}
	...
	public function tags()
	{
		...
		$posts = $this->db->select('keywords')->get('carte')->result();
